...
object Lang extends CMD {
  override def execute(s: CommandSender, args: Array[String]): Boolean = {
    ...
    if (args.length == 1) {
      ...
      if (configs("lang").isPathPresent(args(1))) {
        if (args.length == 2) {
          val permission: String = configs("commands").findPermission(args(0), className, perm = "update.self")
          ...
          val permission: String = configs("commands").findPermission(args(0), className, perm = "update.others")
          if (s.hasPerm(permission)) {
  ...
  override def suggestions(s: CommandSender, args: Array[String]): Array[String] = {
    ...
    argsLength match
      case 2 =>
        suggest = configs("lang").root.keySet().toArray().map(_.toString)
      case 3 =>
